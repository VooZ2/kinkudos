...
|---|---|---:|---|
| [Hostinger VPS](hostinger.md) | People who want the simplest supported Hostinger VPS route | No; use Docker Manager | VPS, updates, and backups |
...
!!! warning "Before you install"
    Generic installations need a 64-bit AMD64 or ARM64 Linux server, Docker Engine, Docker Compose, a hostname, and an HTTPS reverse proxy. The Hostinger path uses Docker Manager and the Traefik proxy installed with its Ubuntu 24.04 with Docker template. Never expose KinKudos port `8000` to the internet.
