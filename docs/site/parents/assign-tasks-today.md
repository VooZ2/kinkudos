...
2. Select catalogue tasks. You may also add one custom title and point value.
3. Decide whether unfinished assigned work should block **new** reward
   requests.
4. Save, then ask the child to open their dashboard and confirm the list is
   visible.
...
- A catalogue task cannot be assigned when it is already pending review,
  already assigned today, or already credited today.
- Unfinished assignments expire at midnight in the server’s local time. They
  remain visible to parents in history but do not award points.
