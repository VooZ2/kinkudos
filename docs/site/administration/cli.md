...
| Send due lottery reminders, assigned-task nudges, and saved-set auto-assign | `docker compose exec -T app python manage.py run_scheduled_reminders` |
...
`run_scheduled_reminders` is the roughly 30-minute timer used for optional lottery reminders, assigned-task nudges, and saved-set auto-assign. It runs those as isolated steps, so one failure does not skip the others. A nudge is sent when an assigned-task batch is still pending about three hours after it was sent, and due saved assignment sets run once per matching local day. The older name `send_lottery_reminders` remains as a legacy alias for existing cron entries.
