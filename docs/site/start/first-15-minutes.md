...
You need one parent account and one child device. Network, email, and remote backups can wait until the basic family flow feels right.

## A working first setup

1. Open **Parents → Settings**. Give the family a name and leave the advanced sections unchanged for now.
2. Under **Parents → Settings → Accounts**, choose **Child profile** in the
   **Create account** panel and create one child. Choose a four-digit PIN and a
   sensible credit value such as `-100` or `0`.
...
![Parent settings with paired devices](../assets/parent-settings-devices-26-6-4.png)

The screenshot contains fictional demonstration data only.

## Add the rest later

- Configure **SMTP** only when parents need password-recovery email or feedback email notifications.
- Configure remote **Backups** once you have a storage provider and a safe place for the repository password.
- Consider **Network access** only if you understand your stable home/VPN addresses. It is useful extra protection, not a prerequisite.

[Pair a child device →](pair-a-child-device.md) · [Parent settings →](../parents/settings.md) · [Lietuviškai](first-15-minutes.lt.md)
