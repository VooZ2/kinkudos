...
| **Daily assigned tasks** | Parents can send a child a set of tasks for today, with optional notes and saved routines that can go out automatically. Completing each one immediately adds the recorded points. |
| **Rewards and goals** | Children request rewards or suggest a new reward/savings goal. Parents decide and set the final point cost. A goal can use available spendable points or save points separately. |
...
| **Scratch tickets** | Optional family-controlled scratch tickets have a configurable cost, per-child weekly limit, outcomes that stay opaque on the board until reveal, and no loss below the lower balance limit. |
...
| **Notifications** | Parents and children can opt into browser notifications for relevant decisions and requests. A child may also receive a gentle reminder about three hours after assigned work is sent, if any item is still waiting. On iPhone and iPad, the app must first be installed to the Home Screen. |
...
[Guided server installer →](../installation/guided-installer.md) · [Your first 15 minutes →](first-15-minutes.md) · [Lietuviškai](what-is-kinkudos.lt.md)
